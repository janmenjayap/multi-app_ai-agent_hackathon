...
export const MODEL_SMOKE_VERSION = 'model-compatibility-smoke-v1';
...
Live calls require an explicit
model ID/key. Mock uses synthetic data and makes zero network requests.
...
export async function runModelSmoke(argv: string[], options: ModelSmokeOptions = {}): Promise<number> {
  ...
  try {
    ...
    receipt.packageVersions = Object.fromEntries(['@langchain/openai', '@langchain/core', 'zod']
    ...
    const mockClient: ModelClient = { mode: 'mock', async dispatchStructured(_request, save) {
      const raw = { status: 200, requestId: null, retryAfterMs: null,
        body: JSON.stringify({ id: 'synthetic-response', status: 'completed', output: [{ type: 'message',
          role: 'assistant', content: [{ type: 'output_text', text: canonical(output) }] }] }) };
      await save(raw);
